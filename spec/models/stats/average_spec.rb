...
RSpec.describe Rates::Average, type: :model do
  describe 'keeping averages cached' do
    ...
    context 'adding exchange for first time' do
      it 'day average is equal to added' do
        exchange_log.save!
        subject = Rates::Average.day(exchange_log.updated)
        expect(subject.usd).to eq exchange_log.usd_rate
      end
      it 'hour average is equal to added' do
        exchange_log.save!
        subject = Rates::Average.hour(exchange_log.updated)
        expect(subject.usd).to eq exchange_log.usd_rate
      end
      it 'minute average is equal to added' do
        exchange_log.save!
        subject = Rates::Average.minute(exchange_log.updated)
        expect(subject.usd).to eq exchange_log.usd_rate
      end
    end
    ...
    context 'more than one exchange in a day' do
      ...
      it 'usd day average is calculated' do
        subject = Rates::Average.day(exchange_log.updated)
        expect(subject.usd).to eq (exchange_log.usd_rate + exchange_log_same_day.usd_rate).fdiv 2
      end
      it 'gbp day average is calculated' do
        subject = Rates::Average.day(exchange_log.updated)
        expect(subject.gbp).to eq (exchange_log.gbp_rate + exchange_log_same_day.gbp_rate).fdiv 2
      end
      it 'eur day average is calculated' do
        subject = Rates::Average.day(exchange_log.updated)
        expect(subject.eur).to eq (exchange_log.eur_rate + exchange_log_same_day.eur_rate).fdiv 2
      end
    end
    ...
    context 'more than one exchange in a hour' do
      ...
      it 'usd hour average is calculated' do
        subject = Rates::Average.hour(exchange_log.updated)
        expect(subject.usd).to eq (exchange_log.usd_rate + exchange_log_same_hour.usd_rate).fdiv 2
      end
      it 'gbp hour average is calculated' do
        subject = Rates::Average.hour(exchange_log.updated)
        expect(subject.gbp).to eq (exchange_log.gbp_rate + exchange_log_same_hour.gbp_rate).fdiv 2
      end
      it 'eur hour average is calculated' do
        subject = Rates::Average.hour(exchange_log.updated)
        expect(subject.eur).to eq (exchange_log.eur_rate + exchange_log_same_hour.eur_rate).fdiv 2
      end
    end
    ...
    context 'more than one exchange in a minute' do
      ...
      it 'usd minute average is calculated' do
        subject = Rates::Average.minute(exchange_log.updated)
        expect(subject.usd).to eq (exchange_log.usd_rate + exchange_log_same_minute.usd_rate).fdiv 2
      end
      it 'gbp minute average is calculated' do
        subject = Rates::Average.minute(exchange_log.updated)
        expect(subject.gbp).to eq (exchange_log.gbp_rate + exchange_log_same_minute.gbp_rate).fdiv 2
      end
      it 'eur minute average is calculated' do
        subject = Rates::Average.minute(exchange_log.updated)
        expect(subject.eur).to eq (exchange_log.eur_rate + exchange_log_same_minute.eur_rate).fdiv 2
      end
    end

    context 'getting average with no data' do
      it 'returns null object for day' do
        subject = Rates::Average.day(Time.current)
        expect(subject.usd).to eq 0
        expect(subject.gbp).to eq 0
        expect(subject.eur).to eq 0
      end
      it 'returns null object for hour' do
        subject = Rates::Average.hour(Time.current)
        expect(subject.usd).to eq 0
        expect(subject.gbp).to eq 0
        expect(subject.eur).to eq 0
      end
      it 'returns null object for minute' do
        subject = Rates::Average.minute(Time.current)
        expect(subject.usd).to eq 0
        expect(subject.gbp).to eq 0
        expect(subject.eur).to eq 0
      end
