...
RSpec.describe Rates::MinMax, type: :model do
  describe 'keeping averages cached' do
    ...
    context 'getting min/max with no data' do
      it 'returns null object for usd min' do
        subject = Rates::MinMax.usd(1.day.ago, Time.current)
      ...
      it 'returns null object for gbp min' do
        subject = Rates::MinMax.gbp(1.day.ago, Time.current)
        expect(subject.min).to eq 0
      end
      ...
      it 'returns null object for eur min' do
        subject = Rates::MinMax.eur(1.day.ago, Time.current)
        expect(subject.min).to eq 0
      end
    ...
    context 'getting min/max with data' do
      ...
      it 'returns min for usd min' do
        subject = Rates::MinMax.usd(1.day.ago, 1.day.from_now)
      ...
      it 'returns min for gbp min' do
        subject = Rates::MinMax.gbp(1.day.ago, 1.day.from_now)
      ...
      it 'returns min for eur min' do
        subject = Rates::MinMax.eur(1.day.ago, 1.day.from_now)
